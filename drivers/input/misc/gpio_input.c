...
{
       setPowerKeyState(0);
       printk(KERN_ERR "[PWR][STATE]power key pressed outdated\n");
}
...
static void handle_power_key_state(unsigned int code, int value)
{
	...
	if (code == KEY_POWER && value == 1) {
		printk(KERN_ERR "[PWR][STATE]try to schedule power key pressed due\n");
		ret = schedule_delayed_work(&power_key_state_disable_work,
		 PWRKEY_PRESS_DUE);
		if (!ret) {
			printk(KERN_ERR "[PWR][STATE]Schedule power key pressed due "
			...
			if (!ret) {
				...
				if (schedule_delayed_work(
				 &power_key_state_disable_work,
				 PWRKEY_PRESS_DUE)) {
					printk(KERN_ERR "[PWR][STATE]Re-schedule power"
					 " key pressed due SCCUESS.\n");
					printk(KERN_ERR "[PWR][STATE] start count for"
				...
				printk(KERN_ERR "[PWR][STATE]Cancel scheduled power"
			...
			printk(KERN_ERR "[PWR][STATE] start count for power key"
			 " pressed due\n");
			setPowerKeyState(1);
		}
	}
...
static void power_key_led_off_work_func(struct work_struct *dummy)
{
	if (power_key_led_requested) {
		if (cancel_delayed_work_sync(&power_key_led_on_work)) {
			printk(KERN_ERR "[PWR] cancel power key led work successfully(%x)\n", power_key_led_requested);
		} else
			printk(KERN_ERR "[PWR] cancel power key led work unsuccessfully (%x)\n", power_key_led_requested);

		power_key_led_requested = 0;
	}
	if (pre_power_key_led_status == 1) {
		printk(KERN_ERR "[PWR] change power key led off\n");
...
static void handle_power_key_led(unsigned int code, int value)
{
	if (code == KEY_POWER) {
		...
		if (value) {
			printk(KERN_ERR "[PWR] start count for power key led on\n");
			schedule_delayed_work(&power_key_led_on_work, PWRKEYLEDON_DELAY);
			power_key_led_requested = 1;
		} else {
			printk(KERN_ERR "[PWR] start count for power key led off\n");
...
static void power_key_check_reset_work_func(struct work_struct *dummy)
{
	...
	if ((aa->clear_hw_reset)) {
		...
		printk(KERN_ERR "[PWR] No reset  clear function\n");
	}
...
static void power_key_clr_check_work_func(struct work_struct *dummy)
{
	if (cancel_delayed_work_sync(&power_key_check_reset_work))
		printk(KERN_ERR "[PWR] cancel power key check reset work successfully\n");
	else
		printk(KERN_ERR "[PWR] cancel power key check reset work unsuccessfully\n");
...
static void handle_power_key_reset(unsigned int code, int value)
{
	if (code == KEY_POWER) {
		if (value) {
			...
			printk(KERN_ERR "[PWR] start count for power key check reset\n");
			if (!schedule_delayed_work(&power_key_check_reset_work, PWRKEYCHKRST_DELAY))
				printk(KERN_ERR "[PWR] the reset work in already in the queue\n");
		} else {
			printk(KERN_ERR "[PWR] start count for power key clear check\n");

			if (!schedule_delayed_work(&power_key_clr_check_work, PWRKEYCLRCHK_DELAY))
				printk(KERN_ERR "[PWR] the clear work in already in the queue\n");
		}
	}
}
#endif
	...
	for (i = 0; i < nkeys; i++, key_entry++, key_state++) {
		...
		if (key_state->debounce & DEBOUNCE_UNSTABLE) {
			...
			if (gpio_flags & GPIOEDF_PRINT_KEY_UNSTABLE)
				printk(KERN_ERR "gpio_keys_scan_keys: key %x-%x, %d "
					"(%d) continue debounce\n",
		...
		if (debounce & DEBOUNCE_POLL) {
			if (pressed == !(debounce & DEBOUNCE_PRESSED)) {
				...
				if (gpio_flags & GPIOEDF_PRINT_KEY_DEBOUNCE)
					printk(KERN_ERR "gpio_keys_scan_keys: key %x-"
		...
		if (gpio_flags & GPIOEDF_PRINT_KEYS)
			printk(KERN_ERR "gpio_keys_scan_keys: key %x-%x, %d (%d) "
				"changed to %d\n", ds->info->type,
...
void keypad_report_keycode(struct gpio_key_state *ks)
{
	...
	if (ds == NULL) {
		printk(KERN_ERR "%s, (ds == NULL) failed\n", __func__);
		return;
	}
	...
	if (key_entry == NULL) {
		printk(KERN_ERR "%s, (key_entry == NULL) failed\n", __func__);
		return;
	}
...
static irqreturn_t gpio_event_input_irq_handler(int irq, void *dev_id)
{
	...
	int pressed;
#endif
	printk(KERN_ERR "%s, irq=%d, use_irq=%d\n", __func__, irq, ds->use_irq);
	...
	if (key_entry->code == KEY_POWER && power_key_intr_flag == 0) {
		...
		printk(KERN_ERR "%s, keycode = %d, first intr", __func__, key_entry->code);
	}
	if (ds->info->debounce_time.tv64) {
		...
		if (ks->debounce & DEBOUNCE_WAIT_IRQ) {
			...
			if (ds->info->flags & GPIOEDF_PRINT_KEY_DEBOUNCE)
				printk(KERN_ERR "gpio_event_input_irq_handler: "
		...
		pressed = gpio_get_value(key_entry->gpio) ^
			!(ds->info->flags & GPIOEDF_ACTIVE_HIGH);
		if (ds->info->flags & GPIOEDF_PRINT_KEYS)
			printk(KERN_ERR "gpio_event_input_irq_handler: key %x-%x, %d "
				"(%d) changed to %d\n",
				ds->info->type, key_entry->code, keymap_index,
				key_entry->gpio, pressed);
		input_event(ds->input_devs->dev[key_entry->dev], ds->info->type,
...
static int gpio_event_input_request_irqs(struct gpio_input_state *ds)
{
	...
	for (i = 0; i < ds->info->keymap_size; i++) {
		...
		if (ds->info->keymap[i].code == KEY_POWER) {
			...
			printk(KERN_ERR "keycode = %d, gpio = %d, type = %lx", ds->info->keymap[i].code, value, req_flags);
		}
		else
		...
		if (err < 0) {
			printk(KERN_ERR "KEY_ERR: %s: request_irq "
		...
		if (ds->info->keymap[i].code == KEY_VOLUMEUP ||
			...
			printk(KERN_ERR "keycode = %d, gpio = %d, irq = %d", ds->info->keymap[i].code, ds->info->keymap[i].gpio, irq);
			if (ds->info->keymap[i].code == KEY_VOLUMEUP)
				vol_up_irq = irq;
		...
		*data = ds = kzalloc(sizeof(*ds) + sizeof(ds->key_state[0]) *
		...
		for (i = 0; i < di->keymap_size; i++) {
			...
			if (dev >= input_devs->count) {
				printk(KERN_ERR "KEY_ERR: %s: bad device "
		...
		for (i = 0; i < di->keymap_size; i++) {
			...
			if (ret) {
				printk(KERN_ERR "KEY_ERR: %s: gpio_request "
		...
		printk(KERN_ERR "GPIO Input Driver: Start gpio inputs for %s%s in %s "
